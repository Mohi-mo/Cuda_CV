...
void onMouseClick(int event, int x, int y, int flags, void* userdata) {
    ...
    if (event == cv::EVENT_LBUTTONDOWN) {
        ...
        if (closestIdx != -1) {
            ...
            std::cout << "X: " << cordsL[closestIdx].x << " Y: " << cordsL[closestIdx].y << " Z: " << data->depth.at<double>(cordsL[closestIdx]) << std::endl;
...
int main(int argc, char** argv) {
    ...
    bool isCalibrate = true;                   // Флаг принудительной калибровки
    ...
    stereo_depth_map(rectifiedLeft, rectifiedRight, P1, P2, stereo_par.T,
                     disparity, SGBM_par.numDisparities, SGBM_par.minDisparity);
    ...
    for (size_t i = 0; i < matches.size(); i++){
        ...
        callbackData.rightKey.push_back(keypointsLeft[matches[i].trainIdx].pt);

        callbackData.leftKey.push_back(keypointsLeft[matches[i].queryIdx].pt);
        callbackData.rightKey.push_back(keypointsLeft[matches[i].trainIdx].pt);

        cv::circle(rectifiedLeft, (callbackData.leftKey[i], callbackData.rightKey[i]), 5, cv::Scalar(150, 150, 150), -1);
    }
