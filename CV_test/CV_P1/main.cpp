...
int main(int argc, char** argv) {
    ...
    */
    ...
    //cv::Mat imageLeft = cv::imread("../../../Fotoset/Stereo/tests/left/R88.png");
    //cv::Mat imageRight = cv::imread("../../../Fotoset/Stereo/tests/right/L88.png");
    ...
    cv::stereoRectify(cameraMatrixL, distCoeffsL, cameraMatrixR, distCoeffsR, imageLeft.size(), R, T, R1, R2, P1, P2, Q);
...
*/
    ...
    cv::Mat disparityMap, disparity;
     cv::Ptr<cv::StereoSGBM> stereo = cv::StereoSGBM::create(0, 16*5, 3);
    //cv::Ptr<cv::StereoSGBM> stereo = cv::StereoSGBM::create(0, 8, 16*5, 0, 1, -1, 31, 15, 0, 0, StereoSGBM::MODE_SGBM_3WAY);
    ...
    // Найдем ключевые точки на левом изображении (пример)
    cv::Mat grayImageLeft;
    cv::cvtColor(imageLeft, grayImageLeft, cv::COLOR_BGR2GRAY);

    std::vector<cv::Point2f> keypoints;
    cv::goodFeaturesToTrack(grayImageLeft, keypoints, imageLeft.cols, 0.01, 10);
